...
def run_e2e_tests():
    ...
    for q_data in original_questions:
        ...
        res = requests.post(f"{API_URL}/eligibility", json={"question": question}, timeout=10)
    ...
    res9 = requests.post(f"{API_URL}/eligibility", json={"question": "  "}, timeout=5)
    ...
    res10 = requests.post(f"{API_URL}/eligibility", json={"question": long_q}, timeout=5)
    ...
    res11 = requests.post(f"{API_URL}/eligibility", json={"question": inj_q}, timeout=5)
    ...
    res12 = requests.post(f"{API_URL}/eligibility", json={"question": lang_q}, timeout=5)
    ...
    # Test 13: Rate Limiting Verification
    print("[13/13] Test 13: Rapid-Fire Rate Limiting Test (Sending 18 rapid requests)...")
    rate_limited_hit = False
    for i in range(18):
        r = requests.post(f"{API_URL}/eligibility", json={"question": f"Rate limit test iteration {i}"}, timeout=5)
